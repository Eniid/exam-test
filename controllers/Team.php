...
class Team
{
    ...
    function viewleague(){
        $client = new Client([ // Création d'un client guzzle
            'base_uri' => "http://domy.be/scores-api/" // on défini l'url de base
        ]);
        
        $response = $client->get('leagues.json'); // requête GET pour la ressource leagues.json
        $body = $response->getBody(); // récupération du "corps", la valeur récupérée par la requête
        $leagues = json_decode($body); // on parse le json récupéré, on en fait un objet utilisable par php


        $view = "views/leagues/view.php"; 

        return compact('view', 'leagues');
    }


    function viewteam(){
        $leaguesname = $_GET['leagueid']; 

        $client = new Client([ // Création d'un client guzzle
            'base_uri' => "http://domy.be/scores-api/" // on défini l'url de base
        ]);
        
        $response = $client->get($leaguesname.'.json'); // requête GET pour la ressource leagues.json
        $body = $response->getBody(); // réupération du "corps", la valeur récupérée par la requête
        $teams = json_decode($body, 1); // on parse le json récupéré, on en fait un objet utilisable par php
